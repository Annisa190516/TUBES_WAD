...
class JoinKelasController extends Controller
{
	...
	public function delete(Request $request){
		$id = JoinKelas::where('id_siswa', $request->id_siswa)->where('id_kelas', $request->id_kelas)->value('id');
		$post = JoinKelas::find($id);

		$post->delete();

		if ($request->auth == "siswa") {
			alert()->success('Berhasil', 'Sekarang anda keluar dari kelas');
